...
# Real tuned thresholds from W3 evaluation ablation 1
...
async def predict_ab(file: UploadFile = File(...)):
    ...
    if not available: raise HTTPException(503, "No models loaded.")
    chosen = random.choice(available)
    image  = Image.open(io.BytesIO(await file.read())).convert("L")
    return {"routed_to": chosen, "ab_test": True,
            **infer(_models[chosen], image)}

@app.post("/predict/{model_name}")
async def predict(model_name: str, file: UploadFile = File(...),
                  tuned_thresholds: bool = True):
    if model_name not in _models:
        raise HTTPException(400, f"Unknown model. Use 'mobilenet' or 'resnet'.")
    if _models[model_name] is None:
        raise HTTPException(503, f"Checkpoint missing. Run restore_checkpoints.py")
    image = Image.open(io.BytesIO(await file.read())).convert("L")
    thr   = TUNED_THRESHOLDS_MV2 if (tuned_thresholds and model_name=="mobilenet") else None
    return {"model": model_name,
            "threshold_mode": "tuned" if thr else "fixed_0.5",
            **infer(_models[model_name], image, thr)}